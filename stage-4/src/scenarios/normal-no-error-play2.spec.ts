// 範例輸入: normal-no-error-play2.in
// 範例輸出: normal-no-error-play2.out
...
describe('normal-no-error-play2 scenario', () => {
  ...
  it('should match the expected game output', async () => {
    const { deckCards, playerNames, actions } = parseScenarioInput(
      'normal-no-error-play2',
    )
    const expectedOutput = readScenarioOutput('normal-no-error-play2')

...
function createOnlyBasicAttackBattle(): RPG {
...
describe('only-basic-attack testcase', () => {
  ...
  it('should match the expected battle output', async () => {
    const actions = readHeroActions('only-basic-attack')
    const expectedOutput = readExpectedOutput('only-basic-attack')
    ...
    const rpg = createOnlyBasicAttackBattle()
    await rpg.battle()
